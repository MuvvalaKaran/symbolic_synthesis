...
class TestBndReachabilityGameArch(unittest.TestCase):
    def test_abstraction(self):
        """
         Check all the tests related Arch abstraction construction for Bounded Human Interventions. 
        """
        # TEST for various formulas 
        formulas = ['F(p00 & p12 & p21) & G(~(p00 & p21) -> ~(p12))',  # arch rigid supports
                    "(F((((p00 & p21) | (p01 & p20)) & p12)) & G((!(((p00 & p21) | (p01 & p20))) -> !(p12))))"] # arch flexible supports


        domain_file_path = PROJECT_ROOT + "/pddl_files/domain.pddl"
        problem_file_path = PROJECT_ROOT + "/pddl_files/problem_arch.pddl"
        
        # correct values
        cor_total_vars: List[int] = [25, 25]
    ...
    def test_synthesis(self):
        """
         Check all the tests related winning strategy synthesis for Bounded Human Interventions with Arch constrcution tasks.
        """
