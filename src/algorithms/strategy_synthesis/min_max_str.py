...
class AdversarialGame(BaseSymbolicSearch):
    ...
    def solve(self, verbose: bool = False):
        ...
        # if verbose:
        #     self.get_prod_states_from_dd(dd_func=self.winning_states[0], sym_lbl_cubes=sym_lbl_cubes, prod_curr_list=prod_bdd_curr_list)
        ...
        while not open_list[g_layer].isZero():
            ...
            if not open_list[g_layer].isZero():
                ...
                if verbose:
                    ...
                    self.get_prod_states_from_dd(dd_func=open_list[g_layer], sym_lbl_cubes=sym_lbl_cubes, prod_curr_list=prod_bdd_curr_list)
                ...
                for tr_idx, tr_action in enumerate(self.ts_transition_fun_list):
                    ...
                    # action_cost = tr_action[0].findMax()
                    ...
                    pred_prod = self.get_pre_states(ts_action=tr_action_bdd, From=open_list[g_layer].bddPattern(), prod_curr_list=prod_bdd_curr_list)
                    ...
                    if step_val in open_list:
                        open_list[step_val] |= prod_image_restricted.existAbstract(self.sys_env_cube)
                    else:
                        open_list[step_val] = prod_image_restricted.existAbstract(self.sys_env_cube)
